#!/usr/bin/env python3
"""Unit and Integration tests for client.GithubOrgClient"""
from typing import Any, Dict, List
import unittest
from unittest.mock import Mock, patch
from parameterized import parameterized, parameterized_class
import client
import utils
import fixtures


class TestGithubOrgClient(unittest.TestCase):
    """Unit tests for GithubOrgClient"""

    @parameterized.expand([
        ("google",),
        ("abc",),
    ])
    @patch("client.get_json")
    def test_org(self, org_name: str, mock_get_json: Mock) -> None:
        """GithubOrgClient.org returns the org payload from get_json"""
        expected = {"org": org_name}
        mock_get_json.return_value = expected
        g = client.GithubOrgClient(org_name)
        self.assertEqual(g.org, expected)
        mock_get_json.assert_called_once_with(client.GithubOrgClient.ORG_URL.format(org=org_name))

    def test_public_repos_url(self) -> None:
        """_public_repos_url returns the repos_url from the org payload"""
        g = client.GithubOrgClient("google")
        fake_org = {"repos_url": "https://api.github.com/orgs/google/repos"}
        with patch.object(client.GithubOrgClient, "org", new_callable=property, return_value=fake_org):
            self.assertEqual(g._public_repos_url, fake_org["repos_url"])

    @patch("client.get_json")
    def test_public_repos(self, mock_get_json: Mock) -> None:
        """public_repos returns a list of repo names, optionally filtered by license"""
        # mock the payload returned by get_json when called with repos url
        mock_get_json.return_value = [
            {"name": "repo1", "license": {"key": "apache-2.0"}},
            {"name": "repo2", "license": {"key": "bsd-3-clause"}},
            {"name": "repo3", "license": None},
        ]
        g = client.GithubOrgClient("google")
        # patch the _public_repos_url property value
        with patch.object(client.GithubOrgClient, "_public_repos_url", new_callable=property, return_value="https://fake.url"):
            repos = g.public_repos()
            self.assertIsInstance(repos, list)
            self.assertListEqual(repos, ["repo1", "repo2", "repo3"])
            # filter by license
            apache = g.public_repos(license="apache-2.0")
            self.assertListEqual(apache, ["repo1"])
        mock_get_json.assert_called_once()

    @parameterized.expand([
        ({"license": {"key": "my_license"}}, "my_license", True),
        ({"license": {"key": "other_license"}}, "my_license", False),
    ])
    def test_has_license(self, repo: Dict[str, Any], license_key: str, expected: bool) -> None:
        """has_license returns True if repo has the given license key"""
        self.assertEqual(client.GithubOrgClient.has_license(repo, license_key), expected)


# Integration tests
param = {
    "org_payload": fixtures.TEST_PAYLOAD[0][0],
    "repos_payload": fixtures.TEST_PAYLOAD[0][1],
    "expected_repos": fixtures.TEST_PAYLOAD[0][2],
    "apache2_repos": fixtures.TEST_PAYLOAD[0][3],
}


@parameterized_class(("org_payload", "repos_payload", "expected_repos", "apache2_repos"), [param])
class TestIntegrationGithubOrgClient(unittest.TestCase):
    """Integration tests for GithubOrgClient.public_repos using fixtures"""

    @classmethod
    def setUpClass(cls) -> None:
        """Start patching requests.get to return fixtures data"""
        # patch requests.get used by utils.get_json
        cls.get_patcher = patch("utils.requests.get")
        mock_get = cls.get_patcher.start()

        def side_effect(url: str, *args, **kwargs):
            """Return a Mock whose json() returns appropriate fixture"""
            if url == cls.org_payload.get("repos_url", ""):
                return Mock(json=Mock(return_value=cls.repos_payload))
            # GitHub org URL
            if url == client.GithubOrgClient.ORG_URL.format(org=cls.org_payload.get("url", "").split("/")[-1]) or url == cls.org_payload.get("repos_url", ""):
                # in some fixture shapes the org payload is passed directly instead:
                return Mock(json=Mock(return_value=cls.org_payload))
            # fallback: return a Mock with empty json
            return Mock(json=Mock(return_value=None))

        mock_get.side_effect = side_effect

    @classmethod
    def tearDownClass(cls) -> None:
        """Stop patching requests.get"""
        cls.get_patcher.stop()

    def test_public_repos(self) -> None:
        """public_repos returns the expected list from fixtures"""
        # in the fixture, org_payload typically contains a 'repos_url' key
        org_name = "google"
        g = client.GithubOrgClient(org_name)
        # patch the ORG_URL formatting only if needed — but utils.requests.get is patched already
        repos = g.public_repos()
        self.assertEqual(sorted(repos), sorted(self.expected_repos))

    def test_public_repos_with_license(self) -> None:
        """public_repos filters by license (apache-2.0)"""
        g = client.GithubOrgClient("google")
        apache = g.public_repos("apache-2.0")
        self.assertEqual(sorted(apache), sorted(self.apache2_repos))
